...
# Build the absolute path to the .txt file
file_path = os.path.join(os.path.dirname(__file__), 'p096_sudoku.txt')

def deduction(A): #inputs a suduko in array form, outputs a new sudoku based on deduction and a true/false. false means a contradiction was found, so the sudoku is invalid.
	while True:
		for i in range(9):
			for j in range(9):
				if A[i][j] == 0:
